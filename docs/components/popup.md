...
通用浮层基础组件。通过 `position` 统摄 **bottom / top / left / right / center / relative** 六种形态，内置滚动锁定（`useScrollLock`）与 `useTeleportTarget`。
...
| `position` | `'bottom' \| 'top' \| 'left' \| 'right' \| 'center' \| 'relative'` | `'bottom'` | 弹层形态与动画 |
